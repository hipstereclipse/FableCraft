...
def write_json(path, data):
    # Names come from the single branding table; structural keys/IDs stay stable.
    _write_json(path, localize(data))
...
def lore_lines(item):
    ...
    return [branded_text(line) for line in lines]
...
def emit_script_data():
    ...
    data = {
        ...
        "augmentInfo": {a["id"].replace("_augment", ""): {"name": a["name"], "desc": a["desc"], "color": a["color"]}
                        for a in fc_data.AUGMENTS if a["id"] != "augment_remover"},
    }
    write_text(BP / "scripts" / "fc_gamedata.js",
               "// AUTO-GENERATED by gen_behavior.py — do not edit.\n"
               "export const DATA = " + json.dumps(localize(data), indent=1) + ";\n")
