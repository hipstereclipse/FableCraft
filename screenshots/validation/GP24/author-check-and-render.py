...
import argparse,copy,hashlib,importlib.util,json,math,sys,types
ROOT=next((p for p in Path(__file__).resolve().parents if (p/'scripts/gen_structures.py').is_file()),Path.cwd())
parser=argparse.ArgumentParser(description='Verify and render the bounded GP24 dining-seat change.')
parser.add_argument('--output',type=Path,default=ROOT/'tmp/conformance/gp24-author',help='Evidence output directory.')
parser.add_argument('--baseline-owner',type=Path,default=ROOT/'tmp/conformance/gp24-author/baseline-gen_structures.py',help='Exact GP23 generator source.')
parser.add_argument('--baseline-asset',type=Path,default=ROOT/'tmp/conformance/gp24-author/baseline-guild.mcstructure',help='Exact GP23 Guild mcstructure.')
parser.add_argument('--references',action='store_true',help='Generate the external-source comparison sheet when both ignored source images are available; otherwise record a skip.')
args=parser.parse_args()
OUT=args.output.resolve();OUT.mkdir(parents=True,exist_ok=True)
BASELINE_OWNER=args.baseline_owner.resolve();BASELINE_ASSET=args.baseline_asset.resolve()
...
before_raw=BASELINE_ASSET.read_bytes();asset=ROOT/'packs/Fablecraft_BP/structures/fc/guild_hall.mcstructure';after_raw=asset.read_bytes()
...
bmod,bgen,brng=build(BASELINE_OWNER,'before');amod,agen,arng=build(ROOT/'scripts/gen_structures.py','after')
...
# Keep historical source hashes even when external pixels are unavailable on a rerun.
reference_index=ROOT/'screenshots/validation/GP23/reference-index.json'
refs=[{k:r[k] for k in ('id','path','sha256','page_url','direct_url')} for r in json.loads(reference_index.read_text())['references'] if r['id'] in {'343658414','236178734'}]
assert len(refs)==2
missing_refs=[r['id'] for r in refs if not (ROOT/r['path']).is_file()]
reference_comparison={'requested':args.references,'generated':False,'missing_source_ids':missing_refs,'historical_record':str(reference_index.relative_to(ROOT)),'historical_record_sha256':sha(reference_index.read_bytes()),'status':'not requested'}
if args.references and not missing_refs:
 for r in refs:assert sha((ROOT/r['path']).read_bytes())==r['sha256']
 ...
 for i,sid in enumerate(('343658414','236178734')):
  path=ROOT/f'tmp/conformance/next-guild-reference/reference-{sid}.jpg';im=Image.open(path).convert('RGB');im.thumbnail((775,480));sheet.paste(im,(15+i*795,50));d.text((20+i*795,540),'Original TLC '+sid+' | distinct stools; exact count/layout unknown',font=small,fill=(40,40,45))
...
Four focused decoded cutaways and, when --references is requested with both source files available, an optional source-comparison sheet show the final result at fixed framing. They omit walls and upper floor, approximate stair/fixture shapes and use flat colors. External pixels and all files here remain ignored scratch. No native lighting, textures, NPC collider execution, walking or saved-world acceptance has run.
...
''')
print(json.dumps({'changed':len(changes),'after_materials':report['after_material_counts'],'old_nodes':len(nodes),'candidate_nodes':len(anodes),'old_reachable':len(reached),'new_reachable':len(areached),'bfs_paths_identical':paths==apaths,'new_gap_sweeps':len(crossings),'negative_fixtures':negative,'source_sha256':report['owner_sha256'],'asset_sha256':report['current_asset_sha256'],'images':[str(p.relative_to(ROOT)) if p.is_relative_to(ROOT) else str(p) for p in images]},indent=2))
