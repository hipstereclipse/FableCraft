...
def main():
    ...
    tracked = [SOURCE, ASSET, 'scripts/fc_lib.py', 'scripts/tests/test_guild_routes.py', 'scripts/gen_screenshots.py',
               ...
               str(Path(__file__).relative_to(ROOT)), str((OUT / 'resident-probe.mjs').relative_to(ROOT))]
    ...
    run = subprocess.run(['node', '--experimental-vm-modules', str(OUT / 'resident-probe.mjs')], cwd=ROOT, input=json.dumps(payload), text=True, capture_output=True)
    ...
    refs = json.loads((OUT / 'reference-reinspection.json').read_text())
    ...
    report = {'baseline_commit': BASE, 'source_sha256': {'before': sha(old_source), 'after': sha(new_source)},
              ...
              'body': {'radius': .4, 'height': 2.1, 'carpet_height': 1 / 16, 'transition_model': 'raise then translate then lower, both directions',
                       'dining_aisles': dining_body, 'new_gap_paths': gaps, 'all_after_body_hits_empty': True, 'baseline_gap_obstructions': baseline_gap_hits},
              'actual_resident_helper': residents, 'resident_colliders': colliders, 'retained_skill_harnesses': harnesses,
              'negatives': negatives, 'references': refs, 'native_acceptance': 'unrun',
