...
export function createDemonDoorPilot({ world, system, ItemStack, report = () => {}, definition = null, placeRoom = null,
  ...
  function readTicket(p) {
    try {
      ...
      const valid = t?.schema === 1 && t.doorId === "guild" && validSource(t.source)
        && Number.isInteger(t.cell) && t.cell >= 0 && t.cell < 4096
        && ["entering", "inside", "outside", "returning"].includes(t.phase);
      return { available: true, ticket: valid ? t : null };
    } catch {
      // A failed read is not evidence that the committed way home is absent.
      // Read-only guards may ignore this player; recovery must defer all writes.
      return { available: false, ticket: null };
    }
  }
  function ticket(p, t = readTicket(p).ticket, r = read()) {
    return t && t.cell === r?.room?.cell && t.source.dimension === r.source.dimension
      && distance(t.source, r.source) < 16 ? t : null;
  }
  function occupiedTicket(p, t = readTicket(p).ticket) {
    try {
  ...
  function requestReturn(p) { return returnFromTicket(p, readTicket(p)); }
  function returnFromTicket(p, current) {
    if (!current.available) return false;
    let r = read();
    let t = ticket(p, current.ticket, r);
    const remembered = occupiedTicket(p, current.ticket);
  ...
  function tick() {
    ...
    for (const p of world.getPlayers()) {
      ...
      try {
        ...
        const remembered = occupiedTicket(p, current.ticket), t = remembered ?? ticket(p, current.ticket, r);
        ...
        const result = inside ? returnFromTicket(p, current) : attemptEntry(p, r, current.ticket);
