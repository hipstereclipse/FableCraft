...
export function createDemonDoorPilot({ world, system, ItemStack, report = () => {}, definition = null, placeRoom = null, sourceReady = () => true }) {
  ...
  function read() {
    const r = parse(world.getDynamicProperty(DOOR_STATE_KEY));
  ...
  function safe(dim, p) {
    ...
    for (const dx of [-0.3, 0.3]) for (const dz of [-0.3, 0.3]) {
      ...
      for (let dy = 0; dy <= 1; dy++) if (!air(blockAt(dim, { x, y: y + dy, z }))) return false;
    }
    return true;
  }
  ...
  function registerGuild(source, face = null, { isNew = false } = {}) {
    ...
    if (face) {
      try { legacyOpen = face.getDynamicProperty("fc_door_open") === true; idx = face.getDynamicProperty("fc_door_idx") ?? null; } catch { }
    }
  ...
  function requestReturn(p) {
    ...
    if (!r) {
      ...
      t = rawTicket(p);
      if (!t || p.dimension.id !== dimension().id || !inRealm(p.location, realmOrigin(t.cell))) return false;
  ...
  function occupiedRealm(p) {
    const r = read();
    return !!r?.room && p.dimension.id === dimension().id && inRealm(p.location, r.room.origin);
  }
  ...
  function tick() {
    ...
    for (const p of world.getPlayers()) {
      ...
      try {
        ...
        const nearSource = p.dimension.id === r.source.dimension && distance(p.location, r.source) < 8;
        const anchor = inside ? add(r.room.origin, ARCANUM.exit) : r.source;
